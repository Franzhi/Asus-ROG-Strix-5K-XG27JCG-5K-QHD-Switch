...
# Мета: Визначити поточну роздільну здатність. Якщо ми ВЖЕ у 2K, 
# скрипт завершує роботу БЕЗ запиту прав Адміністратора.
# ======================================================================
$is5K = $false
$videoControllers = Get-CimInstance Win32_VideoController
foreach ($vc in $videoControllers) {
    if ($vc.CurrentHorizontalResolution -eq 5120) {
        $is5K = $true
        break
    }
}
...
if (-not $is5K) {
    exit # Ми вже у 2K (або іншому режимі), негайний тихий вихід
}
...
# ======================================================================
# БЛОК 5: ВІДПРАВКА АПАРАТНИХ КОМАНД (ПЕРЕМИКАННЯ НА 2K)
# ======================================================================
$vcpJoystick = 0xEB
